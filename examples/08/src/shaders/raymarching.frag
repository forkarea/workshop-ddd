...
float belt(vec3 posBox) {
	posBox.xz = rotate(posBox.xz, posBox.y * 0.2);
	posBox.xy = repAng(posBox.xy, 200.0);
	posBox.y -= 3.5;

	float dBox = box(posBox, 0.75);
	return dBox;
}
...
vec2 map(vec3 pos) {
	...
	float d = sphere(pos, 5.0) + displacement(pos) * 1.5;

	vec3 posBox = pos;
	posBox.z = repeat(posBox.z + uGlobalTime, .5);
	float dBox = box(posBox, vec3(15.0, 15.0, thickness));
	d = max(d, dBox);

	float t = floor((pos.z + uGlobalTime) / 0.5);
	colorIndex = mod(t, 2.0) < 1.0 ? 0.0 : 1.0;
