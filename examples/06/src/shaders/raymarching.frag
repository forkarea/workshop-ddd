...
vec3 repAng(vec2 p, float n) {
    float ang = 2.0*PI/n;
    float sector = floor(atan(p.x, p.y)/ang + 0.5);
    p = rotate(p, sector*ang);
    return vec3(p, sector*ang);
}
...
vec2 map(vec3 pos) {
	...
	float d = 0.0;
	float dSphere = sphere(pos, 1.5);

	vec3 pRing = pos;
	vec3 rotResult = repAng(pRing.xy, 500.0);
	pRing.xy = rotResult.xy;
	pRing.y -= 5.0;
	pRing.yz = rotate(pRing.yz, rotResult.z + uGlobalTime);
	float dBox0 = box(pRing, boxSize);
	float dBox1 = box(pRing * vec3(1.01, 1.01, 1.0), boxSize);
	d = min(dBox0, dBox1);
	d = min(d, dSphere);

	if(d == dBox1) colorIndex = 0.0;
