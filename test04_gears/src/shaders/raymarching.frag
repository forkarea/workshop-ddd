...
vec2 map(vec3 pos) {
	...
	float d = cylinder(pos, vec2(diskSize, diskWidth/2.0));
	float cInner = cylinder(pos, vec2(diskSize - diskWidth, 1.1));
	d = max(d, -cInner);

	vec3 posBox = pos;
	posBox.xz = repAng(posBox.xz, 12.0);
	posBox.z -= diskSize + diskWidth * .49;
	
	float dBox = box(posBox, diskWidth * .5);
	if(d >= dBox) {
		colorIndex = 0.0;
	}
	d = min(d, dBox);
